...
router.post('/register', multipartMiddleware, async function (req, res) {
  ...
  if (!imageFileName) {
    let error_msg = {
      "msg": "Please select a profile picture for upload",
      "param": "uploadPicture"
    };
    if (!errors) {
      errors = [];
    ...
    if (ext != "png" && ext != "jpg") {
      let error_msg = {
        "msg": "Profile picture must be a .png or .jpg file",
  ...
  if (newUser.user_id.length === 0) {
    let error_msg = {
      "msg": "Valid username not provided",
      "param": "user_id"
    };
    if (!errors) {
      errors = [];
  ...
  if (newUser.password.length === 0) {
    let error_msg = {
      "msg": "Valid password not provided",
      "param": "password"
    };
    if (!errors) {
      errors = [];
  ...
  if (newUser.dob.length === 0) {
    let error_msg = {
      "msg": "Valid date of birth not provided",
      "param": "dob"
    };
    if (!errors) {
      errors = [];
  ...
  if (newUser.location.length === 0) {
    let error_msg = {
      "msg": "Valid location not provided",
      "param": "location"
    };
    if (!errors) {
      errors = [];
  ...
  if (newUser.occupation.length === 0) {
    let error_msg = {
      "msg": "Valid occupation not provided",
      "param": "occupation"
    };
    if (!errors) {
      errors = [];
